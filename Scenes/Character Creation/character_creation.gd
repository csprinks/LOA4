...
## One hero's creation panel: portrait, name, two class dropdowns, and Attribute
## Point allocation across the six stats. Built programmatically so all four panels
## are identical and easy to restyle, and themed to match the hero cards. Exposes
## the accessors the creation coordinator (character_creation_flow.gd) reads on
## "Start Game".
##
## Attribute Point rules (see "Class Jobs List"): every stat starts at 10; the hero
## has 10 Attribute Points; each point raises a stat by its Gain, which depends on
...
const GAIN_OVERLAP := 6
const GAIN_PRIMARY := 4
const GAIN_SECONDARY := 2
const GAIN_OFF_CLASS := 1

const FONT := 16
const PORTRAIT_DIR := "res://Portraits/"
const PORTRAIT_EXTENSIONS := ["png", "jpg", "jpeg", "webp", "bmp", "svg"]

var points_spent: Dictionary = {}
var primary_class := ""
var secondary_class := ""
...
func _build() -> void:
	...
	_atr_label = _label("Attribute Points (ATR): %d" % TOTAL_ATTRIBUTE_POINTS, 18)
	col.add_child(_atr_label)

	# Stat allocation grid: Stat | Total | - | spent | + | Gain
	...
	for header in ["Stat", "Total", "", "ATR", "", "Gain"]:
	...
	for stat in STAT_ORDER:
		...
		var gain := _label(str(GAIN_OFF_CLASS))
		_gain_labels[stat] = gain
		grid.add_child(gain)
...
# --- Rules ------------------------------------------------------------------
func gain_per_point(stat: String) -> int:
	...
	return GAIN_OFF_CLASS


func points_remaining() -> int:
	var spent := 0
	for stat in STAT_ORDER:
		spent += points_spent[stat]
	return TOTAL_ATTRIBUTE_POINTS - spent
...
func _refresh() -> void:
	for stat in STAT_ORDER:
		var gain := gain_per_point(stat)
		_total_labels[stat].text = str(BASE_STAT + int(points_spent[stat]) * gain)
...
func get_stat_data() -> Dictionary:
	...
	for stat in STAT_ORDER:
		out[stat] = {
			"base": BASE_STAT,
			"points_spent": int(points_spent[stat]),
			"gain_per_point": gain_per_point(stat),
		}
	return out
